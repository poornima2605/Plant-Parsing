...
class Parsing(torch.nn.Module):
    def __init__(self, cfg, dim_in, spatial_in):
        ...
        if self.parsingiou_on:
            self.ParsingIoU = ParsingIoU(cfg, self.Head.dim_out, 
                                         self.Head.spatial_out)
    ...
    def _forward_train(self, conv_features, targets=None):
        ...
        loss_parsing, parsingiou_targets = self.loss_evaluator(
            logits, targets['parsing'])
        losses.update(dict(train_loss_parsing=loss_parsing))

        if self.parsingiou_on:
            loss_parsingiou, _ = self.ParsingIoU(x, parsingiou_targets)
            losses.update(dict(train_loss_parsingiou=loss_parsingiou))

        return None, losses

    def _forward_test(self, conv_features, targets):
        x = self.Head(conv_features)
        logits = self.Output(x)
        #print("logits",logits)
        ...
        results = dict(
            probs=output,
            parsing_iou_scores=torch.ones(output.size()[0], 
                                          dtype=torch.float32, 
        ...
        if targets != None:
            loss_parsing, parsingiou_targets = self.loss_evaluator(
                logits, targets[0].parsing)
            losses.update(dict(loss_parsing=loss_parsing.item()))

            if self.parsingiou_on:
                loss_parsingiou, _ = self.ParsingIoU(x, parsingiou_targets)
                losses.update(dict(loss_parsingiou=loss_parsingiou.item()))
