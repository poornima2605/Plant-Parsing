...
class ResNet(res.ResNet):
    def __init__(self, cfg, stride=32):
        ...
        #self.layer1 = self._make_layer(block, 128, layers[0], 1, conv=stage_with_conv[0], ctx=stage_with_ctx[0])
        #self.layer2 = self._make_layer(block, 256, layers[1], 2, conv=stage_with_conv[1], ctx=stage_with_ctx[1])
        #self.layer3 = self._make_layer(block, 512, layers[2], 2, conv=stage_with_conv[2], ctx=stage_with_ctx[2])
        #self.layer4 = self._make_layer(block, 1024, layers[3], 2, conv=stage_with_conv[3], ctx=stage_with_ctx[3])
