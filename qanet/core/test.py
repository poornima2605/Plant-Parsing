...
class TestEngine(object):
    def __init__(self, cfg, model):
        ...
        if is_main_process():
            ...
            if self.cfg.VIS.ENABLED:
                ...
                if not os.path.exists(os.path.join(self.cfg.CKPT, 'vis_inst')):
                    mkdir_p(os.path.join(self.cfg.CKPT, 'vis_inst'))
    ...
    def __call__(self, inputs, targets):
        if self.cfg.TEST.INSTANCES_PER_BATCH == -1:
            ...
            #print("target_list",targets_list.size())
    ...
    def parsing_inference(self, targets):
        ...
        aug_idx = 0
        size = [self.cfg.TEST.SCALE[1], self.cfg.TEST.SCALE[0]]
        outputs, losses = self.model.parsing_net(self.features[aug_idx], targets)
        ...
        parsing_loss = torch.tensor([losses['loss_parsing']])
        parsing_loss_iou = torch.tensor([losses['loss_parsingiou']])
        self.result['parsing'] = dict(
            ...
            loss_parsing = [parsing_loss],
            loss_parsingiou = [parsing_loss_iou]
...
# =============================================================================
#             inputs = [torch.from_numpy(_).float() for _ in inputs]  # _.transpose(2, 0, 1)
...
# =============================================================================
            ...
            for i, target in enumerate(targets):
                box = target.bbox.clone()
                box[:, 2:4] *= scale
                batch_inds = torch.ones(len(box)).to(dtype=box.dtype)[:, None] * i
                rois.append(torch.cat([batch_inds, box], dim=1))
            inputs_list =[]
            print("inputs", np.array(inputs).size())
            for i in rois:
                if len(rois) == 0:
                   return None
                inputs_list.append(
                   roi_align_rotated(np.array(inputs), i, (self.cfg.TEST.SCALE[1], 
                                                            self.cfg.TEST.SCALE[0]), 1.0, 0, True))
            ...
            image_list = np.asarray(inputs_list).transpose(0, 3, 1, 2)
            print("image_list",image_list.shape)
            inputs = image_list.to(torch.device(self.cfg.DEVICE))
            
        elif self.cfg.TEST.AFFINE_MODE == 'cv2':
            assert scale == 1.0, 'cv2 only supports single scale'
            inputs = list(inputs)
            inputs = [_ for _ in inputs if len(_) > 0]
            if len(inputs) == 0:
                return None
            inputs = torch.cat(inputs, dim=0).to(torch.device(self.cfg.DEVICE)).float().contiguous()
