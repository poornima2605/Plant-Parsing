...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hi:u:o:s:", ["annotation_file=", "input_folder", "output_folder=", "superclass"])

        if len(opts)< 4:
            print('usage: python Hasty_to_COCO.py -i <input_file1> -u <input_file2> -o <output_file> -s <supercategory>')
            sys.exit(2)

    except getopt.GetoptError:
        print('python Hasty_to_COCO.py -i <input_file1> -u <input_file2> -o <output_file> -s <supercategory>')
        sys.exit(2)
    ...
    for opt, arg in opts:
        ...
        elif opt in ("-i", "--input_file1"):
            in_path1 = arg
        
        elif opt in ("-u", "--input_file2"):
            in_path2 = arg
    ...
    with open(in_path1) as in_file_handle1, open(in_path2) as in_file_handle2:

        in_file1 = json.load(in_file_handle1) # Reading json file
        in_file2 = json.load(in_file_handle2)
        class_idx=0
        for in_file in (in_file1,in_file2):
            
             label_classes = in_file1["label_classes"]
             ...
             for label_class  in  label_classes :
                ...
                #class_id_asgmt[label_class["class_name"]] = class_idx
                     ...
                     if label_class["class_name"]!= superclass :
                         ...
                         class_id_asgmt.append(label_class["class_name"])
                    
                     elif label_class["class_name"]== superclass:
                    
                         class_id_asgmt.append("Background")

                #if label_class["class_name"]!= superclass:
                     category_subsect["parsing"] = class_id_asgmt
                     category_subsect["name"] = label_class["class_name"]
                     category_subsect["id"] = class_idx 

                     if (label_class["class_name"]== superclass) and (in_file==in_file1) :
        ...
        # Introducing some counters for indexing duties
             ...
             for img in tqdm.tqdm(uqe_imgs):
                 ...
                 image_subsect["height"] = height
                 image_subsect["width"] =  width
