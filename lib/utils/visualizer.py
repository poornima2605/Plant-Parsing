...
class Visualizer(object):
    ...
    def vis_preds(self, boxes=None, classes=None, masks=None, keypoints=None, parsings=None, uvs=None, hiers=None,
                  semsegs=None, panos=None, panos_label=None, gnd_bboxes = None, part_scores= None):
        ...
        sorted_inds = (-boxes[:,-1]).argsort()
        #print("sorted later", sorted_inds)
        #timers['bbox_prpoc'].toc()
        

        instance_id = 1
        for i in sorted_inds:
            bbox = boxes[i, :-1]
            gnd_bbox= gnd_bboxes[i, :-1]
            score = boxes[i, -1]
            part_score = part_scores[i]
            #print("part score", part_score)
            ...
            if self.cfg.SHOW_BOX.ENABLED:
                if len(bbox) == 4:
                    timers['show_box'].tic()
                    gnd_bbox_x0 = gnd_bbox[0] - 0.5 *(gnd_bbox[2] - gnd_bbox[0])
                    gnd_bbox_y0 = gnd_bbox[1] - 0.5 * (gnd_bbox[3] - gnd_bbox[0])
                    #self.vis_bbox((gnd_bbox_x0, gnd_bbox_y0, gnd_bbox[2] - gnd_bbox[0], gnd_bbox[3] - gnd_bbox[1]), (0,0,255))
                    self.vis_bbox((bbox[1], bbox[0], bbox[3]-bbox[1], bbox[2]-bbox[0]), ins_color)
                    timers['show_box'].toc()
            ...
            if self.cfg.SHOW_CLASS.ENABLED:
                ...
                self.vis_class((bbox[1], bbox[0] - 2), class_str, ins_color, part_score)
        ...
        for box in boxes[:,:-1]:
            instance=self.im[int(min(box[0],box[2])):int(max(box[0],box[2])), int(min(box[1],box[3])):int(max(box[1],box[3]))]
    ...
    def vis_class(self, pos, class_str, bg_color, part_score):
        """Visualizes the class."""
        font_color = self.cfg.SHOW_CLASS.COLOR
        font_scale = self.cfg.SHOW_CLASS.FONT_SCALE
        
        colormap = self.get_colormap(self.cfg.SHOW_PARSS.COLORMAP)
        colormap = colormap_utils.dict2array(colormap)
        keys = ['Non-viable', 'Tip Cutting', 'Redundant Top End', 'First-section Cutting', 'Second-section Cutting', 'Third-section Cutting', 'Redundant Bottom End','Remains']
        txt2 = dict(zip(keys, np.round(np.array(part_score),2).T))
        
        non_zero_elements = np.count_nonzero(part_score)
        ...
        back_tl = x0, y0 - int((non_zero_elements +2.5) * txt_h)
        back_br = x0 + txt_w *2, y0
        cv2.rectangle(self.im, back_tl, back_br, (170,170,170), -1)
        
        # Show text.
        txt_tl = x0, y0 - int((non_zero_elements+ 1.5) * txt_h)
        cv2.putText(self.im, txt, txt_tl, font, font_scale, (0,0,0), lineType=cv2.LINE_AA)
        
        j=0
        for i, (part, score) in enumerate(txt2.items()):
            if score !=0.0:
               txt_tl = x0, y0 - int((non_zero_elements-j*1.3) * txt_h)
               color = ( int (colormap [ i+1,0 ]), int (colormap [i+1, 1 ]), int (colormap [ i+1,2 ])) 
               cv2.putText(self.im, str(part)+':'+ str(score), txt_tl, font, font_scale, tuple(color), lineType=cv2.LINE_AA)
               j=j+1
