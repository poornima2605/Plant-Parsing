...
from lib.utils.comm import gather, is_main_process
from lib.utils.events import (CommonMetricPrinter, EventStorage,
                                  EventWriter, JSONWriter, TensorboardXWriter)
from lib.utils.misc import logging_rank
from lib.utils.net import get_bn_modules, update_bn_stats
...
def write_metrics(metrics_dict, storage):
    ...
    if is_main_process():
        ...
        const_keys = ("val_parsing_loss", "val_parsingiou_loss")
        for m_k in const_keys:
            if m_k in all_metrics_dict[0]:

                m_v = [x.pop(m_k) for x in all_metrics_dict]
                storage.put_scalar(m_k, m_v[0])
                
        const_keys1 = ("best_acc1")
        for m_k in const_keys1:
            if m_k in all_metrics_dict[0]:
        ...
        total_losses_reduced = sum(v if 'train_loss' in k else 0 for k, v in metrics_dict.items())

        storage.put_scalar("total_loss", total_losses_reduced)
        
        total_losses_reduced_val = sum(v if 'val_parsing' in k else 0 for k, v in metrics_dict.items())

        storage.put_scalar("val_total_loss", total_losses_reduced_val)
