...
def write_metrics(metrics_dict, storage):
    ...
    if is_main_process():
        max_keys = ("data_time", "best_acc1","val_parsing_loss", "val_parsingiou_loss","val_total_loss" )
        for m_k in max_keys:
        ...
        #total_losses_reduced = sum(v if k =='train_loss_parsing' or 'train_loss_parsingiou' else 0 for k, v in metrics_dict.items())
        total_losses_reduced = (v if 'train_loss_parsing' in k else 0 for k, v in metrics_dict.items()) + (v if 'train_loss_parsingiou' in k else 0 for k, v in metrics_dict.items())
        storage.put_scalar("total_loss", total_losses_reduced)
        
        # validation_loss_parsing = (v if 'val_parsing_loss' in k else 0 for k, v in metrics_dict.items())
        # storage.put_scalar("val_loss_parsing", validation_loss_parsing)
        # validation_loss_parsingiou = (v if 'val_parsingiou_loss' in k else 0 for k, v in metrics_dict.items())
        # storage.put_scalar("val_loss_parsingiou",validation_loss_parsingiou)
        
        # storage.put_sclar("val_total_loss",validation_loss_parsing+validation_loss_parsingiou )
        # total_val_losses_reduced = (v if 'val_parsing_loss' in k else 0 for k, v in metrics_dict.items()) + (v if 'val_loss_parsingiou' in k else 0 for k, v in metrics_dict.items())
        # storage.put_scalar("total_val_loss", total_val_losses_reduced)
