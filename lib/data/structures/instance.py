...
class Instance(object):
    def __init__(self, bbox, image_size, labels, ann_types=None, instances=None):
        ...
        if 'parsing' in self.ann_types:
            ...
            #self.parsing_instances = self.instances['parsing'].parsing
    ...
    def crop_and_resize(self, train_size, affine_mode='cv2'):
        self.trans = get_affine_transform_modified(self.bbox, train_size) if affine_mode == 'cv2' else None
        for ann_type in self.ann_types:
            self.instances[ann_type].crop_and_resize(self.bbox, train_size, self.trans)


    def generate_target(self, target_type='gaussian', sigma=2, prob_size=(192, 192), 
                        train_size=1024):  #, new_target=[], new_bbox = torch.tensor([])):
        target = {}
        #result_target=[]
        if 'mask' in self.ann_types:
        ...
        if 'parsing' in self.ann_types:
            #if new_target == []:
            #print("parsing",self.instances['parsing'].parsing)
            target['parsing'] = self.instances['parsing'].parsing.long()
            
               #self.parsing_instances= target['parsing'] 
            # else:
            #     target['parsing']= new_target
            #     self.instances['parsing'].parsing = new_target
            #     x0, y0, w, h = new_bbox[:4]
            #     xc = x0 + w * 0.5
            #     yc = y0 + h * 0.5
            #     self.bbox = torch.tensor([xc,yc,w,h,0.])
    ...
    def update_target_bbox(self, new_target=[], new_bbox = torch.tensor([])):
        target = {}
        target['parsing']= new_target
        self.instances['parsing'].parsing = new_target
        x0, y0, w, h = new_bbox[:4]
        #xc = x0 + w * 0.5
        #yc = y0 + h * 0.5
        self.bbox = torch.tensor([x0,y0,w,h,0.])
        return target
    ...
    def __repr__(self):
        ...
        s += 'instance={})'.format(np.shape(self.instances['parsing'].parsing))
        return s
    
    def __call__(self):
        return self.instances['parsing'].parsing, self.bbox
...
class Parsing(object):
    ...
    def __init__(self, parsing_list):
        ...
        self.parsings.append(self.parsing)
    ...
    def flip(self, image_w=None):
        ...
        #print("Flip",self.parsing)
    
    def RanRotate90(self):
        Rot90 = A.RandomRotate90(p=1)
        print(self.parsing.shape)
        augmented = Rot90(mask=self.parsing)
        self.parsing = augmented['mask']
    ...
    def crop_and_resize(self, bbox, train_size, trans):
        if trans is None:
            ...
            self.parsing = roi_align_rotated(
                parsing[None, None], rois, (train_size[1], train_size[0]), 0.5, 1, True, "nearest"
            ).squeeze()
            # x0 = bbox[0] - bbox[2]*0.5
            # y0 = bbox[1] - bbox[3]*0.5
            # xw = x0+ bbox[2]
            # yh = y0 + bbox[3]
            # self.parsing = parsing[int(x0):int(xw),int(y0):int(yh)]
        else:
...
def get_affine_transform_modified(box, output_size): 
    ...
    src[0, 0] = center[0] - src_w * 0.5
    src[0, 1] = center[1] - src_h * 0.5
    src[1, 0] = center[0] + src_w * 0.5  
    src[1, 1] = center[1] - src_h * 0.5  
    src[2, 0] = center[0] - src_w * 0.5 
    src[2, 1] = center[1] + src_h * 0.5 
