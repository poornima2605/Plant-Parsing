    ...
    def __getitem__(self, idx):
        ...
        if 'mask' in self.ann_types:
            if isinstance(ann['segmentation'], list):
                ...
                instance['mask'] = os.path.join(self.root.replace(
                    'images', 'masks'), ann['segmentation'])
        ...
        if 'uv' in self.ann_types:
            instance['uv'] = [ann['dp_x'], ann['dp_y'], ann['dp_I'], 
                              ann['dp_U'], ann['dp_V'], ann['dp_masks']]
...
# =============================================================================       
        ...
        if self.transforms is not None:           
            ...
            #print("target",target,self.transforms)
...
class COCOInstanceTestDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #if (file_name == "stn2_pkg032_0_0402_rep.png"):
            #print("img_id",img_id)
            #print("anno",anno)
            #print("boxes",boxes)
        ...
        parsing = [[self.root, file_name, obj['parsing_id']] for obj in anno]
        #print("parsing",parsing)

        target = InstanceBox(boxes, classes, (img.shape[1], img.shape[0]), scores, parsing=parsing)
