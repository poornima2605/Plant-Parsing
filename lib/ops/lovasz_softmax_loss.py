...
def lovasz_softmax_loss(x, target, ignore_index=None, only_present=True):
    x = F.softmax(x, dim=1)
    total_loss = 0
    N = x.shape[0]
    for inp, tgt in zip(x, target):
        total_loss += lovasz_softmax_flat(inp, tgt, ignore_index, only_present)
    return total_loss / N
