...
def train(cfg, model, sampler, train_loader, test_loader, test_set, 
    ...
    scheduler.info['val_loss_parsing']=0
    scheduler.info['val_loss_parsingiou'] =0
    metrics_dict1={}
    # main loop
        ...
        try:
            ...
            for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHS + 1):
                ...
                for iteration in range(0, iter_per_epoch):
                    ...
                    metrics_dict["best_acc1"] = scheduler.info['cur_acc']
                    
                    metrics_dict['val_parsing_loss'] = scheduler.info['val_loss_parsing']
                    metrics_dict['val_parsingiou_loss'] = scheduler.info['val_loss_parsingiou']
                ...
                if cfg.TEST.DATASETS and cfg.TEST.FREQUENCY_EPOCHS != -1 and epoch % cfg.TEST.FREQUENCY_EPOCHS == 0:
                    ...
                    cur_acc, cur_loss, cur_loss_iou = test(cfg, 
                                                           test_loader, 
                                                           test_set, model)
                    if is_main_process():
                        scheduler.info['cur_acc'] = cur_acc * 100
                        scheduler.info['val_loss_parsing'] = cur_loss
                        scheduler.info['val_loss_parsingiou'] = cur_loss_iou
                        write_metrics(metrics_dict, storage)

                        logging_rank(
                            'Evaluating at epoch {} is done, acc1 is {:4.2f}%, val parsing loss is {:4.2f}, and val parsing iou loss is {:4.2f} .'
                            .format(epoch, scheduler.info['cur_acc'], torch.median(scheduler.info['val_loss_parsing']), torch.median(scheduler.info['val_loss_parsingiou']))
                        )
...
def test(cfg, loader, datasets, model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            loss_parsing.append((result['parsing']['loss_parsing'][0]))
            loss_parsingiou.append((result['parsing']['loss_parsingiou'][0]))
    ...
    loss_parsing = torch.cat(loss_parsing)
    loss_parsingiou = torch.cat(loss_parsingiou)
    print("loss_parsing",loss_parsing)
    print("loss_parsingiou",loss_parsingiou)
    ...
    if is_main_process():
        ...
        return eval.evaluation(datasets, all_results) , torch.median(loss_parsing), torch.median(loss_parsingiou)
    else:
        return 0.
...
def main(args):
    ...
    #Creating Validation Loader and engine
    ...
    # Train
    train(cfg, model, train_sampler, train_loader, test_loader, test_set, 
          optimizer, scheduler, scaler, checkpointer, all_hooks)
